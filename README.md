...
- [ ] **Binary protocol.** Replace diff-only JSON with a packed binary frame
  (MessagePack or a fixed `ArrayBuffer` layout). Drop each diff from ~60 bytes to
  ~12, removing JSON parsing on mobile as well.
...
- [x] **Search.** Filter the watchlist by symbol or name, off the rAF path so live ticks
  keep flowing while typing.

- [x] **Sorting.** Sort by price, day-change %, or other variables, including live re-sorting as values tick (with a toggle).
...
- [ ] **Ticker symbols & icons.** Add ticker symbols and icons to help present tokens better and increase recognizability.
